/**
	...
	***************************************************************************/
	function printESCP() {
		...
		qz.appendImage("http://wasabisushi.fr/admin/img/image_sample_bw.png", "ESCP", "single");
	...
	***************************************************************************/ 
	function printPDF() {
		...
		qz.appendPDF("misc/pdf_sample.pdf");
	...
	***************************************************************************/ 
	...
	function printNihao(data){
		var noRepeat = 0;
		if (data != null) {
			data = JSON.parse(data);

			qz.appendImage("http://noboru91.fr/admin/image/risheng.png", "ESCP", "single");
				//qz.append(chr(27)+chr(33)+chr(0));

			window["qzDoneAppending"] = function() {
				qz.append(chr(27)+chr(97)+chr(1));
				qz.append('\n'+data[0].store_name+'\n');
				qz.append('01 60 86 09 09 / 06 60 25 04 26 \n');
				qz.append(chr(27)+chr(97)+chr(48)+'\n\n');
	            		//qz.append(chr(27)+chr(69)+chr(10));
	            		qz.append('Date              '+data[0].date_added+'\n\n');
	            		qz.append('No                '+data[0].order_id+'\n\n');
	            		qz.append('PRENOM            '+data[0].payment_firstname+'\n');
	            		qz.append('NOM               '+data[0].payment_lastname+'\n\n');
	            		qz.append('TEL               '+data[0].telephone+'\n\n');
	            		qz.append('PAIEMENT          '+data[0].payment_method+'\n\n');
	            		//qz.append('TOTAL   \t'+parseFloat(data[0].total)+' Euros\n\n');
	            		qz.append('Adress:\n');
	            		qz.append('   \t'+data[0].payment_address_1+'\n')
	            		qz.append('   \t'+data[0].payment_address_2+'\n');
	            		qz.append('   \t'+data[0].payment_city+' '+data[0].payment_postcode+'\n');
	            		qz.append('Batiment          '+data[2].batiment+'\n');
	            		qz.append('Escalier          '+data[2].escalier+'\n');
	            		qz.append('Interphone        '+data[2].interphone+'\n');
	            		qz.append('Ascenceur         '+data[2].ascenceur+'\n');
	            		qz.append('DigiCode1         '+data[2].digicode1+'\n');
	            		qz.append('DigiCode2         '+data[2].digicode2+'\n');
	            		qz.append('Etage             '+data[2].etage+'\n');
	            		qz.append('Porte             '+data[2].porte+'\n\n');
	            		
	            		qz.append('COMMENTAIR:\t\n');
	            		qz.append('        '+data[0].comment+'\n\n\n');

	            		//qz.append(chr(27)+chr(33)+chr(0));
	            		qz.append('------------------------------------------\n------------------------------------------\n');
	            		qz.append('QTT X NOM\t\t\tTOTAL\n\n');

	            		for(var j = 0; j < data[1].length; j++){
	            			if (data[1][j].name.length > 27) {
	            				qz.append(data[1][j].quantity+'\n\t\t\t\t'+data[1][j].name+'\n');
	            				if (data[1][j].ordermeals) {
	            					for (var i = 0; i < data[1][j].ordermeals.split(',').length; i++) {
	            						qz.append("----"+data[1][j][i].name+'\n');
	            					};
	            					console.log("entering ordermeals");
	            				}else{
	            					console.log("not entering");
	            				};
	            				qz.append('------------------------------------------\n');
	            			}else if (data[1][j].name.length > 19) {
	            				qz.append(data[1][j].quantity+'\t'+data[1][j].name+'\n');
	            				if (data[1][j].ordermeals) {
	            					for (var i = 0; i < data[1][j].ordermeals.split(',').length; i++) {
	            						qz.append("----"+data[1][j][i].name+'\n');
	            					};
	            				...
	            				qz.append('------------------------------------------\n');
	            			}else if (data[1][j].name.length > 11) {
	            				qz.append(data[1][j].quantity+'\t\t'+data[1][j].name+'\n');
	            				if (data[1][j].ordermeals) {
	            					for (var i = 0; i < data[1][j].ordermeals.split(',').length; i++) {
	            						qz.append("----"+data[1][j][i].name+'\n');
	            				...
	            				qz.append('------------------------------------------\n');
	            			}else if (data[1][j].name.length > 3) {
	            				qz.append(data[1][j].quantity+'\t\t\t'+data[1][j].name+'\n');
	            				if (data[1][j].ordermeals) {
	            					for (var i = 0; i < data[1][j].ordermeals.split(',').length; i++) {
	            						qz.append("----"+data[1][j][i].name+'\n');
	            ...
	            qz.append('------------------------------------------\n');
	            qz.append(chr(27)+chr(97)+chr(50));
	            qz.append('TOTAL:\t'+parseFloat(data[0].total)+' euros\n');
	            qz.append(chr(27)+chr(97)+chr(48));
	            qz.append('------------------------------------------\n');
	            qz.append(chr(27)+chr(97)+chr(49));
	            qz.append('----Merci de votre comande et a bientot---\n');
	            qz.append(chr(27)+chr(97)+chr(48));
	            qz.append('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n');
	            qz.append(chr(27) + chr(105));
	...
	function printSecond(data){
		console.log("entering second print");
		var noRepeat = 0;
		qz.appendImage("http://dreamsushi94.fr/admin/img/d1.png", "ESCP", "single");
				//qz.append(chr(27)+chr(33)+chr(0));

			window["qzDoneAppending"] = function() {
				qz.append(chr(27)+chr(97)+chr(1));
				qz.append('\n'+data[0].store_name+'\n');
				qz.append('01 56 34 07 13 \n');
				qz.append(chr(27)+chr(97)+chr(48)+'\n\n');
	            		//qz.append(chr(27)+chr(69)+chr(10));
	            		//qz.append('Date              '+data[0].date_added+'\n\n');
	            		qz.append('No                '+data[0].order_id+'\n\n');
	            	...
	            	if (noRepeat == 0) {
	            		...
	            		for(var j = 0; j < data[1].length; j++){
	            			if (data[1][j].name.length > 27) {
	            				qz.append(data[1][j].quantity+' X '+data[1][j].name+'\n\t\t\t\t'+parseFloat(data[1][j].total)+'\n');
	            				qz.append('------------------------------------------\n');
	            			}else if (data[1][j].name.length > 19) {
	            				qz.append(data[1][j].quantity+' X '+data[1][j].name+'\t'+parseFloat(data[1][j].total)+'\n');
	            				qz.append('------------------------------------------\n');
	            			}else if (data[1][j].name.length > 11) {
	            				qz.append(data[1][j].quantity+' X '+data[1][j].name+'\t\t'+parseFloat(data[1][j].total)+'\n');
	            				qz.append('------------------------------------------\n');
	            ...
	            qz.append('------------------------------------------\n');
	            qz.append(chr(27)+chr(97)+chr(49));
	            qz.append('----Merci de votre appeler et a bientot---\n');
	            qz.append(chr(27)+chr(97)+chr(48));
	            qz.append('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n');
	            qz.append(chr(27) + chr(105));
