				...
				<table class='cart_class'>
					...
					<?php foreach ($productss as $product) {?>
						...
						<?php if ($product['model'] != 'additional') { 
							$number_of_piece += $product['quantity'];//这是计算出了筷子之类的东西外的总数量
						//如果不是甜的和咸的，就显示?>
						
						<tr class="toRemove" id="p<?=$product['key']?>">
							<td><?php echo $product['name']?><br><span class='ordermeals'>
								<?php 
									if (count($product['meals'])-1) {
										$isMeal = true;
							...
							<td >
								...
								<?php }else{?>
									<span class="minusCart" onclick="minusCart(<?=$product['key']?>,<?=$product['quantity']?>)" name="<?=$product['key']?>" value="<?=$product['quantity']?>"><i class="fa fa-minus"></i></span>&nbsp;&nbsp;
									<span><?=$product['quantity']?></span>&nbsp;&nbsp;
									<span class="plusCart" onclick="plusCart(<?=$product['key']?>,<?=$product['quantity']?>)" name="<?=$product['key']?>" value="<?=$product['quantity']?>"><i class="fa fa-plus"></i></span>
								<?php }?>
							</td>
							<td><div class="delete_cart_menu" onclick="deleteCart(<?=$product['key']?>)"name="<?=$product['key']?>"><i class="fa fa-trash-o"></i></div></td>
						</tr>
					...
					<tr class="tr_special">
						...
						<td class="opacity0">Graduit</td><!--
						<td>
							<input type="number" class="update-cart input-small form-control" name="quantity[633]" id="633" value="<?php 
						...
						<td id="bagu_id">
							<span class="minusCart" onclick="minusCart(633,<?=$p_de_bagu_qty?>)" name="633" value="<?=$p_de_bagu_qty?>"><i class="fa fa-minus"></i></span>&nbsp;&nbsp;
							<span ><?=$p_de_bagu_qty?></span>&nbsp;&nbsp;
							<span class="plusCart" onclick="plusCart(633,<?=$p_de_bagu_qty?>)" name="633" value="<?=$p_de_bagu_qty?>"><i class="fa fa-plus"></i></span>
						</td>-->
						<td id="bagu_id">
							<span onclick="updateToCart(633,<?php 
								if ($p_de_bagu_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(633)" name="633"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					<tr class="tr_special">
						<td>Sauce Sucre</td>
						<td class="opacity0">Graduit</td><!--
						<td>
							<input type="number" class="update-cart input-small form-control" name="quantity[634]" id="634" value="<?php
							if (isset($sucre_qty)) {
								echo $sucre_qty;
							}else{
								echo 0;
						...
						<td id="sucre_id">
							<span class="minusCart" onclick="minusCart(634,<?=$sucre_qty?>)" name="634" value="<?=$sucre_qty?>"><i class="fa fa-minus"></i></span>&nbsp;&nbsp;
							<span ><?=$sucre_qty?></span>&nbsp;&nbsp;
							<span class="plusCart" onclick="plusCart(634,<?=$sucre_qty?>)" name="634" value="<?=$sucre_qty?>"><i class="fa fa-plus"></i></span>
						</td>-->
						<td id="sucre_id">
							<span onclick="updateToCart(634,<?php 
								if ($sucre_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="minusCart(634)" name="634"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					<tr class="tr_special">
						<td>Sauce Sale</td>
						<td class="opacity0">Graduit</td><!--
						<td>
							<input type="number" class="update-cart input-small form-control" name="quantity[635]" id="635" value="<?php
							if (isset($sale_qty)) {
								echo $sale_qty;
							}else{
								echo 0;
						...
						<td id="sale_id">
							<span class="minusCart" onclick="minusCart(635,<?=$sale_qty?>)" name="635" value="<?=$sale_qty?>"><i class="fa fa-minus"></i></span>&nbsp;&nbsp;
							<span ><?=$sale_qty?></span>&nbsp;&nbsp;
							<span class="plusCart" onclick="plusCart(635,<?=$sale_qty?>)" name="635" value="<?=$sale_qty?>"><i class="fa fa-plus"></i></span>
						</td>-->
						<td id="sale_id">
							<span onclick="updateToCart(635,<?php 
								if ($sale_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(635)" name="635"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					...
					<tr class="tr_special">
						...
						<td class="opacity0">Graduit</td><!--
						<td>
							<input type="number" class="update-cart input-small form-control" name="quantity[241]" id="241" value="<?php
						...
						<td id="gin_id">
							<span class="minusCart" onclick="minusCart(241,<?=$gingembre_qty?>)" name="241" value="<?=$gingembre_qty?>"><i class="fa fa-minus"></i></span>&nbsp;&nbsp;
							<span ><?=$gingembre_qty?></span>&nbsp;&nbsp;
							<span class="plusCart" onclick="plusCart(241,<?=$gingembre_qty?>)" name="241" value="<?=$gingembre_qty?>"><i class="fa fa-plus"></i></span>
						</td>-->
						<td id="gin_id">
							<span onclick="updateToCart(241,<?php 
								if ($gingembre_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(241)" name="241"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					<tr class="tr_special">
						<td>Wasabi</td>
						<td class="opacity0">Graduit</td><!--
						<td>
							<input type="number" class="update-cart input-small form-control" name="quantity[242]" id="242" value="<?php
							if (isset($wasabi_qty)) {
								echo $wasabi_qty;
							}else{
								echo 0;
						...
						<td id="was_id">
							<span class="minusCart" onclick="minusCart(242,<?=$wasabi_qty?>)" name="242" value="<?=$wasabi_qty?>"><i class="fa fa-minus"></i></span>&nbsp;&nbsp;
							<span ><?=$wasabi_qty?></span>&nbsp;&nbsp;
							<span class="plusCart" onclick="plusCart(242,<?=$wasabi_qty?>)" name="242" value="<?=$wasabi_qty?>"><i class="fa fa-plus"></i></span>
						</td>-->
						<td id="was_id">
							<span onclick="updateToCart(242,<?php 
								if ($wasabi_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(242)" name="242"><i class="fa fa-trash-o"></i></div></td>
					</tr><!--
					<tr>
						...
						<td class="font_big" id="money_in_total"><?php echo $totals[1]['text'];?></td>
					</tr>-->
				</table>
				<div class="clear"></div>
				<div class="row" style="float:right;text-align:right">
					<div class="col-md-4 font_big">TOTAL</div>
					<div class="col-md-8 font_big" id="money_in_total"><?php echo $totals[1]['text'];?></div>
				</div>
	...
	<div class="viewport">
		<section class="formPage">
						...
						<div class="row" id="cate_row_restr_width_2">
							<div class="navbar-header">
								...
								<div class="login-suc-div" id="not_home_login" >
										<ul id="loginornot" class="mainNav">
											<?php if (!$logged) { ?>
							...
							<div class="collapse navbar-collapse">
								<ul class="nav navbar-nav navbar-right mainNav" id="mainNav">
									<li>
										<a href="http://<?=$_SERVER["SERVER_NAME"]?>/index.php?route=common/buffet">Specialite</a>
									</li>
			...
			<div class="container">
				<div class="row" id="cate_row_restr_width">
					...
					<div class="carteRight col-md-7">
						...
						<h3 style="color:#1b97c5;text-indent:7%">
						<?php 
							if (isset($child_cat_id)) {
								foreach ($categories as $category) {
									if ($category["category_id"] == $parent_cat_id) {
			...
			<?php 
				...
				$week = date('w');
				$t = date('H:i');
				$t = floatval(str_ireplace(':', '.', $t));
				$settime_am = explode('-',$setting["config_horaire_AM_{$week}"]);
				$settime_am[0] = floatval(str_ireplace(':', '.', $settime_am[0]));
				$settime_am[1] = floatval(str_ireplace(':', '.', $settime_am[1]));
				$settime_pm = explode('-',$setting["config_horaire_PM_{$week}"]);
				$settime_pm[0] = floatval(str_ireplace(':', '.', $settime_pm[0]));
				$settime_pm[1] = floatval(str_ireplace(':', '.', $settime_pm[1]));
				if(( $t >= $settime_am[0] && $t <= $settime_am[1] ) || ( $t >= $settime_pm[0] && $t <= $settime_pm[1] )){ 
		 	?>
