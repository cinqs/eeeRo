...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->get['remove'])) {
			...
			unset($this->session->data['meal'][$this->request->get['remove']]);
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$this->data['full']=$this->model_checkout_order->getFull();

			//echo "<pre>";print_r($this->data['full']);echo "</pre>";exit;
	...
	public function add() {
		...
		if (isset($this->request->post['temp'])) {
			$temp = $this->request->post['temp'];
		} else {
			$temp = 3;
		}
		$meal ='';
		if(isset($this->request->post['meal'])){
			$meal = rtrim($this->request->post['meal'],',');
		}
		...
		if(!empty($product_info['setmeal'])){
			$meals =explode(',',$meal);
			$setmeal = explode(',',$product_info['setmeal']);
			if(count($meals) != count($setmeal)){
				echo json_encode('-1');   //传过来的菜单与设置的不同
				exit;
			}
		}
		...
		if ($product_info) {			
			...
			if (!$json) {
				$temp_result=$this->model_catalog_product->getDisponible($temp);
				if($temp_result=="false"){
					echo json_encode($json[$temp_result]);
					exit;
				}
				$this->cart->add($this->request->post['product_id'], $quantity, $option,$meal);
	...
	public function update(){
		...
		if (!$_POST["quantity"] && !empty($product_info['setmeal'])) {
			$_SESSION['meal'][$_POST['product_id']] = '';
		}

		if (isset($_POST["product_id"]) && isset($_POST["quantity"])) {
			$result = $this->cart->update($_POST["product_id"], $_POST["quantity"]);
	...
	public function full(){
		$this->session->data['full']=$this->request->post['full'];
	}
